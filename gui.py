...
class Receptor:
    ...
    def dnd_commit(self,Source,Event):
        ...
        print("Accepting Frame Number = %s" % str(self.guiInst.get_frame_num(Event.x)))
        ...
        if isinstance(Source, Dragged):
            ...
            if(self.guiInst.get_frame_num(Event.x) in self.guiInst.frame_dict and not self.guiInst.frame_dict.get(self.guiInst.get_frame_num(Event.x)).ListChildren):     # If there are children in frame, it already contains an instruction
                self.guiInst.create_instruction_frame(self.guiInst.get_frame_num(Event.x), Event.widget.cget('text'))
            else:
...
class gui():
    ...
    def create_instruction_frame(self, frame_num, button_name):
        frame = self.frame_dict.get(frame_num)
        ...
        remove.pack()
        remove.bind('<ButtonPress>',lambda event: self.remove_frame_children(event, frame))
        
        #Create edit button
        edit = Button(frame, text='Edit')
        edit.pack()
        edit.bind('<ButtonPress>', lambda event: self.settings_popup(event, frame))
    ...
    def remove_frame_children(self, Event, frame):
    ...
    def settings_popup(self, Event, frame):
        widget_list = list()    #Use on toplevel window close to 'get()' all the slider properties and pass them to frame.instruction
        toplevel = Toplevel()    
        toplevel.protocol("WM_DELETE_WINDOW", lambda: self.extract_instructions(frame, widget_list, toplevel))  #On popup close, call extract_instructions with frame info and widget_list.
    ...
    def extract_instructions(self, frame, widget_list, window):
    ...
    def on_dnd_start(self, Event, button):
        """
    ...
    def start_drawing(self):
        ...
        def update(ind):
            ...
            if(not self.inst_thread.isAlive()):
                print("Killed window")
                toplevel.destroy()
...
if __name__ == "__main__":
    ...
    Start.bind('<ButtonPress>', lambda event: guiInst.start_drawing())
    ...
    Motors.pack(fill='x')
    Motors.bind('<ButtonPress>', lambda event: guiInst.on_dnd_start(event, 'Motors'))
    HeadTilt = Button(frame,text='HeadTilt')
    HeadTilt.pack(fill='x')
    HeadTilt.bind('<ButtonPress>', lambda event: guiInst.on_dnd_start(event, 'HeadTilt'))
    HeadTurn = Button(frame,text='HeadTurn')
    HeadTurn.pack(fill='x')
    HeadTurn.bind('<ButtonPress>',lambda event: guiInst.on_dnd_start(event, 'HeadTurn'))
    BodyTurn = Button(frame,text='BodyTurn')
    BodyTurn.pack(fill='x')
    BodyTurn.bind('<ButtonPress>',lambda event: guiInst.on_dnd_start(event, 'BodyTurn'))
...
#    Introduce.bind('<ButtonPress>',lambda event: guiInst.on_dnd_start(event, 'Introduce'))      #Allow me to introduce myself, I am CL4P-TP
    ...
    guiInst.frame_dict = {1:frame1,2:frame2,3:frame3,4:frame4,5:frame5,6:frame6,7:frame7,8:frame8}
